...
type Tree = { __parent: Tree | undefined } & (
  | Partial<Value>
  | { [key: string]: Tree }
);
...
const findPath = (
  ...
  if (seg && seg.length > 0) {
    const findSeg = (tree: Tree): Value | undefined =>
      seg.reduce((t: Tree, s: string | number) => {
        if (!t) {
          return undefined;
        }
    ...
    const result = { ...retVal, scale: seg.join(".") };
    if (typeof retVal.__value === "string") {
      const p = findPath(fullTree, currentNode, retVal.__value);
      if (p) {
...
const transformValue = (
  spaceFormats: PluginOptions["spaceFormats"],
  key: string,
  value: any
) => {
  if (spaceFormats[key]) {
...
const transformTree = (props: TransformProps) => {
  ...
  Object.keys(current)
    .filter((key) => !["__parent", "__path", "__value"].includes(key))
    .forEach((key) => {
      ...
      if ("__parent" in item) {
        ...
        if (Array.isArray(item.__value)) {
          ...
          if (
            micromatch.isMatch(key, colorNames) &&
            fullTree["colors"]?.[item.__value]
          ) {
            if (rootNames.includes(parentKey)) {
              ...
              item.__path.node.value.value =
                fullTree["colors"]?.[item.__value].__value;
            }
          } else {
          ...
          switch (typeof __value) {
            case "object":
              if (!Array.isArray(__value)) {
                item.__path.node.value = transformValue(
                  spaceFormats,
                  scale,
                  __path.node.value
                );
              } else {
                item.__path.node.value.elements = item.__path.node.value.elements.map(
                  (e, idx) => ({
                    ...e,
                    value:
                      __value[idx] !== undefined
                        ? transformValue(
                            spaceFormats,
                            scale,
                            ((__value[idx] as unknown) as Value).__value
                          )
            ...
            case "string":
            case "number":
              item.__path.node.value.value = transformValue(
                spaceFormats,
                scale,
                __value
              );
...
const nativeColorNames = Object.keys(scales).filter(
  (key) => scales[key] === "colors"
);
...
export default (_: any, options: PluginOptions) => {
  ...
  const colorNames = transformNativeColors
    ? [...customColorNames, ...nativeColorNames]
    : customColorNames;
